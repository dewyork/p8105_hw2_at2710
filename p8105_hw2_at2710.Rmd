...
class:question,
sample_size,
confidence_limit_low:geo_location)) %>% mutate(resposne_binary = ifelse(response %in% c('Excellent','')))
...
**Answer** Data comes from `r length(unique(health_overall_wide_fmt$state))` states with `r names(sort(table(health_overall_wide_fmt$state), decreasing = TRUE)[1])` being observed the most (`r sort(table(health_overall_wide_fmt$state), decreasing = TRUE)[1]` times).
...
health_overall_wide_fmt %>% filter(year == 2002) %>% ggplot(aes(x = Excellent)) + geom_histogram()
```

Make a scatterplot showing the proportion of “Excellent” response values in New York County and Queens County (both in NY State) in each year from 2002 to 2010.
```{r scatterplot, message = FALSE}
# NY_dat <- health_overall_wide_fmt %>% filter(locationdesc %in% c('NY - Queens County', 'NY - New York County')) %>% 
#   ggplot
